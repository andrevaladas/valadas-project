...
public class LoginFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (request instanceof HttpServletRequest && response instanceof HttpServletResponse) {
			...
			String paginaLogin = "/principal/gtt.jsp";
	...
	throws IOException, ServletException {
        ...
        // Valida se o usuário está acesando a página de login ou inclusão/manutenção de usuario stgp caso positivo, não deve validar nada
        boolean val = true;
        if (pagina.equals(paginaLogin)) {
        	val = false;
        } else if (pagina.endsWith("/pnsI099.jsp")) {
        	val = false;
        } else if (pagina.endsWith("/pnsM099.jsp")) {
        	val = false;
        }
        if (val) {
