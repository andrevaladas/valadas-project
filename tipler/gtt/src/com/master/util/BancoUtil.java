...
import com.master.util.bd.BeanProcessor;
 ...
 */
public class BancoUtil extends Transacao {
	...
	/**
	 * Retorna curval da sequencia solicitada
	 */
	public long getSeq(String seq) throws Excecoes {

		ResultSet res = null;
		try {

			query = " select last_value as nrSeq from " + seq  ;

			res = this.sql.executarConsulta(query);

			if (res.next()) { return  res.getLong("nrSeq");	}

		} catch (SQLException exc) {
			throw new Excecoes(exc.getMessage(), exc, this.getClass().getName(), "getSeq(String seq)");
		} finally {
	 ...
	 */
	public long getSeqNextval(String sequenceName) throws Excecoes {
		ResultSet res = null;
		try {
			query = " SELECT nextval('"+sequenceName+"') ";
			res = this.sql.executarConsulta(query);
			if (res.next()) { return  res.getLong("nextval");	}
		} catch (SQLException exc) {
			throw new Excecoes(exc.getMessage(), exc, this.getClass().getName(), "getSeqNextval(String sequenceName)");
		} finally {
			this.closeResultset(res);
		}
		return 0;
	 ...
	 */
	public String getTableStringValue(String campo, String from, String where)
			throws Excecoes {
	 ...
	 */
	public int getTableIntValue(String campo, String from, String where)
			throws Excecoes {
	 ...
	 */
	public double getTableDoubleValue(String campo, String from, String where)
			throws Excecoes {
	 ...
	 */
	public int getMaximo(String campo, String from, String where)
			throws Excecoes {
	 ...
	 */
	public String getMaximoData(String campo, String from, String where)
			throws Excecoes {
	...
	public void executarUpdate(String query) throws Excecoes {
		try {
			...
			throw new Excecoes(exc.getMessage(), exc,
					this.getClass().getName(), "executarUpdate()");
		}
	}
	...
	public boolean doValidaData(String parametro) {
		return JavaUtil.doValidaData(parametro);
	}
	...
	/**
	 * Verifica se - O ID foi excluído por outro usuário - Já existe outro ID
	 * com o mesmo código
	 */
	public void doValidaUpdate(int oid, String cd, String table,
			String fieldOid, String fieldCd) throws Excecoes {
		String from = table;
		String where = fieldOid + " = " + oid;
		// Valida se o registro em edição foi excluído por outro usuário
		if (doExiste(from, where)) {
			where = fieldOid + " <> " + oid + " and " + fieldCd + " = '" + cd
					+ "'";
			...
			throw new Excecoes(
					"ID informado para atualização não existe (pode ter sido "
	...
	public void closeResultset(ResultSet res) {
		if (res != null) {
			...
			try {
				...
				// No driver jdbc mais novo, o statement é fechado com o
