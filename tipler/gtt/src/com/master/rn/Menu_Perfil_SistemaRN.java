...
public class Menu_Perfil_SistemaRN extends Transacao {
    ...
    public void processaOL(String acao, Object Obj, HttpServletRequest request, HttpServletResponse response)
		...
		if ("M".equals(acao)) {
            ...
            mpED.setDm_Lingua(usuario.getDm_Lingua());
            // Prepara a saída do menu dele....
            out.println("<menu>");
			String saida = ""; boolean flagTopic, flagSection, flagItem, flagSubItem;
			flagTopic = false ; flagSection = false ; flagItem = false; flagSubItem = false;
		...
		if ("L".equals(acao)) {
			...
			for (int i=0; i<lst.size(); i++){
				...
				int p = edVolta.getNm_Tela().indexOf("C.lzx");
				if (p>0){
					saida += "nm_Tela='"+edVolta.getNm_Tela().substring(p-6,p)+"' " ;
				} else {
					saida += "nm_Tela=' ' ";
				}
