    ...
    public void processaOL(String acao, Object Obj, HttpServletRequest request, HttpServletResponse response)
    	...
    	if ("I".equals(acao) ) {
    		if (checkDuplo(ed,acao)) {  
    			out.println("<ret><item oknok='Fabricante já existente com esta descrição !'/></ret>");
    		} else {
	    		ed = this.inclui(ed);
		...
		if ("D".equals(acao)) {
			if (checkEmUso(ed)) {
				out.println("<ret><item oknok='Impossível excluir! Fabricante em uso!' /></ret>");
			} else {
				this.delete(ed);
		...
		for (int i=0; i<lst.size(); i++){
			...
			if ("L".equals(acao)) {
				...
				saida += "cd_Fabricante_Banda='" + edVolta.getCd_Fabricante_Banda() + "' ";
