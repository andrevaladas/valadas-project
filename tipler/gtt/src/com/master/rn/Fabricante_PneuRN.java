...
public class Fabricante_PneuRN extends Transacao {
    ...
    public void relatorio(Fabricante_PneuED ed, HttpServletRequest request, HttpServletResponse response) throws Excecoes {
        try {
			...
			ed.setNomeRelatorio("gtt004"); // Seta o nome do relatório
			// Monta a descricao do filtro utilizado
			if (Utilitaria.doValida(ed.getNM_Fabricante_Pneu()))
    ...
    public boolean checkEmUso ( Fabricante_PneuED ed ) throws Excecoes {
		try {
			Modelo_PneuED mpED = new Modelo_PneuED();
			mpED.setOid_Fabricante_Pneu(ed.getOid_Fabricante_Pneu());
			this.inicioTransacao();			
			return (new Modelo_PneuRN(this.sql).lista(mpED).size()>0 ? true : false); 
        } finally {
            this.fimTransacao(false);
        }
    }
