...
public class PneuBD extends BancoUtil{
  ...
  public void alteraMotivoSucata (PneuED ed) throws Excecoes {
	  ...
	  sql += " WHERE " +
		  ...
		  throw new Excecoes (e.getMessage () , e , this.getClass ().getName () , "alteraMotivoSucata(PneuED ed)");
	  }
  }
  ...
  public void altera (PneuED ed) throws Excecoes {
	  ...
	  try {
		  ...
		  ",dt_nota_fiscal ='" + ed.getDt_Nota_Fiscal() + "'"     +
		  ",nr_serie ='" + (doValida(ed.getNr_Serie()) ? ed.getNr_Serie() : "" ) + "'" +	
		  ",nr_Fogo ='" + ed.getNr_Fogo() + "'"     +
		  ",cd_item_estoque ='" + ed.getCd_Item_Estoque() + "' " +
  ...
  public void alteraModelo (PneuED ed) throws Excecoes {
	  try {
		  ...
		  throw new Excecoes (e.getMessage () , e , this.getClass ().getName () , "alteraModelo (PneuED ed)");
	  }
  }
  ...
  public void alteraSituacaoPneu (PneuED ed) throws Excecoes {
   ...
   String sql = " UPDATE "+
	        "pneus "+
	        " SET "+
	        "cd_pneu = '"+ ed.getCD_Pneu ()+ "'"+
	        ",nr_fabrica ="+ ed.getNR_Fabrica ()+
			",dt_stamp = '"+ ed.getDt_stamp()+ "'" +
			",usuario_stamp = '"+ ed.getUsuario_Stamp()+ "'" +  
			",dm_stamp = '"+ ed.getDm_Stamp()+ "'" +
	        ",oid_fabricante_pneu ="+ ed.getOid_Fabricante_Pneu () +
	        ...
	        ",oid_tipo_pneu ="+ ed.getOid_Tipo_Pneu()+
	        ",oid_modelo_pneu ="+ ed.getOid_Modelo_Pneu () +
	        ",dm_localizacao ='"+ ed.getDM_Localizacao ()+ "'"+
	        ",dm_situacao ='"+ ed.getDM_Situacao ()+ "'" +
	        ",nr_km_acumulada ="+ ed.getNr_Km_Acumulada()+    
	        ",tx_lonas ="+ ed.getTx_Lonas()+ "'" +
	        ",nr_nota_fiscal ="+ ed.getNr_Nota_Fiscal() +
	        ",nr_perimetro ="+ ed.getNr_Perimetro()+
	        ",tx_dot ="+ ed.getTx_Dot() + "'" +
	        ",dt_nota_fiscal ="+ ed.getDt_Nota_Fiscal() + "'" +
	        ",nr_serie ="+ ed.getNr_Serie() + "'" +	
	        ",nr_Fogo ="+ ed.getNr_Fogo() + "'" +
			",oid_dimensao_pneu = "+ ed.getOid_Dimensao_Pneu() +
			",cd_item_estoque ="+ ed.getCd_Item_Estoque() +
	        ",vl_preco ="+ ed.getVl_Preco() +	
	...
	public void delete (PneuED ed) throws Excecoes {
		try {
			...
			if (ed.getOid_Empresa()>0)
				sql +=" oid_empresa = "  + ed.getOid_Empresa() +" and " ;
			sql +="oid_pneu = "  + ed.getOid_Pneu();  
			executaSQL.executarUpdate (sql);
		}
		catch (SQLException e) {
   ...
   * @throws Excecoes
   */
  public ArrayList<PneuED> listaGroupByDFM (PneuED ed) throws Excecoes {
	  ArrayList<PneuED> list = new ArrayList<PneuED> ();
	  sql= "SELECT " +
	  "nm_Dimensao_Pneu," ;
	...
	public ArrayList<PneuED> Busca_Eixo_Veiculo(PneuED ed) throws Excecoes {
		  ArrayList<PneuED> list = new ArrayList<PneuED> ();
		  sql = "SELECT DISTINCT " +
		  "dm_eixo " +
		  "FROM " +
	...
	public ArrayList<PneuED> Busca_Pneu (PneuED ed) throws Excecoes {
		ArrayList<PneuED> list = new ArrayList<PneuED> ();
		try {
	...
	public ArrayList<PneuED> listaPorMarca (PneuED ed) throws Excecoes {
		ArrayList<PneuED> list = new ArrayList<PneuED> ();
		sql = "SELECT " +
	...
	// Lista para a Tela Pneus por Veiculo
	public ArrayList<PneuED> listaPorVeiculo (PneuED ed) throws Excecoes {
		...
		" FROM " +
		" pneus as p " +
		" left join veiculos as v on (v.oid_veiculo = p.oid_veiculo) " +
		" left join modelos_veiculos as mv on (v.oid_modelo_veiculo = mv.oid_modelo_veiculo) " +
		" left join tipos_veiculos as tv on (tv.oid_tipo_veiculo = mv.oid_tipo_veiculo) " +
		" left join fabricantes_pneus as f on (p.oid_fabricante_pneu =f.oid_fabricante_pneu) " +
		" left join modelos_pneus as m on (p.oid_modelo_pneu = m.oid_modelo_pneu) " +
		" left join dimensoes_pneus as d on (p.oid_dimensao_pneu = d.oid_dimensao_pneu) " +
		" WHERE " ;			        
		sql+=" p.oid_Empresa = " + ed.getOid_Empresa() + " and " ;
		if (ed.getOid_Tipo_Veiculo()>0 ) {
			sql+=" mv.oid_tipo_veiculo =  " + ed.getOid_Tipo_Veiculo() +" and ";
		}
		sql+=" ( p.oid_veiculo is not null and p.oid_veiculo<>'' ) " +
		" ORDER BY " +
		" v.nr_Frota," +
		" p.dm_Posicao";
		System.out.println("frota pneuBD listaporveiculo sq="+sql);
		ResultSet rs = this.executaSQL.executarConsulta(sql);
		try {
			while (rs.next ()) {
				PneuED popula = new PneuED ();
				popula.setNR_Frota(rs.getString("nr_Frota"));
				popula.setNm_Tipo_Veiculo(rs.getString("nm_tipo_veiculo"));
				popula.setNr_Fogo(rs.getString("nr_Fogo"));
				popula.setDt_Entrada(FormataData.formataDataBT(rs.getString("dt_Entrada")));
				popula.setNr_Hodometro_Veiculo(rs.getDouble("nr_Hodometro_Veiculo"));
				popula.setDM_Posicao(rs.getString("dm_Posicao"));
				popula.setNm_Fabricante_Pneu(rs.getString("nm_fabricante_pneu"));
				popula.setNm_Dimensao_Pneu(rs.getString("nm_dimensao_pneu"));
				popula.setNm_Modelo_Pneu(rs.getString("nm_modelo_pneu"));
			...
			throw new Excecoes (e.getMessage () , e , this.getClass().getName(), "listaPorVeiculo ()");
		}
	}	  

	
	public ArrayList<PneuED> listaRecusasRecapagens (PneuED ed) throws Excecoes {
		ArrayList<PneuED> list = new ArrayList<PneuED> ();
		sql = "SELECT "+
		"p.*, " +
		"fo.nm_razao_social " +
		...
		"fornecedores fo " +
		"WHERE " +
		"fo.oid_fornecedor = p.oid_fornecedor_recusa_recapagem " +
		"AND " +
  ...
  public ArrayList<PneuED> lista (PneuED ed) throws Excecoes {
	  ...
	  "FROM " +
	  "pneus p " +
	  "left join veiculos v on v.oid_veiculo = p.oid_veiculo " +
	  "left join fabricantes_pneus f on p.oid_fabricante_pneu = f.oid_fabricante_pneu " +
	  "left join modelos_pneus m on p.oid_modelo_pneu = m.oid_modelo_pneu " +
	  //"left join dimensoes_pneus d on p.oid_dimensao_pneu = d.oid_dimensao_pneu " +
	  //"left join tipos_pneus t on p.oid_tipo_pneu = t.oid_tipo_pneu " +
	  //"left join fornecedores fo  on p.oid_fornecedor = fo.oid_fornecedor  " +
	  "WHERE " +
	  " 1=1 " ;
	  if (ed.getOid_Pneu () > 0) { // Se informado o oid_Pneu busca somente por isso.... 
		  ...
		  if ("SUCATEAMENTO".equals(ed.getDt_Sucateamento())) { // Busca somente pneus sucateados e ainda não vendidos
			  sql += " and dt_Sucateamento is not null ";
			  
			  //if ( !"VENDIDOS".equals(ed.getDm_CP_Selecao()) )
				//  sql += " and dt_venda is null ";
			  if (ed.getOid_Unidade_Sucateamento()>0)
				  sql += " and p.oid_Unidade_Sucateamento = " + ed.getOid_Unidade_Sucateamento()+ " ";
		  ...
		  if (doValida (ed.getOid_Veiculo ())) {
			  sql += " AND p.oid_veiculo = '" + ed.getOid_Veiculo () + "'";
		  }
	  ...
	  try {
		  //System.out.println("STGP PneuBD lista dt_estoque ="+ed.getDt_Estoque() +" - sql="+sql);
		  ResultSet rs = this.executaSQL.executarConsulta(sql);
		  while (rs.next ()) {
 	  ...
 	  try {
		  while (rs.next ()) {
			  ...
			  if ( ("STP1".equals(rs.getString("dm_Posicao")))  || ("STP2".equals(rs.getString("dm_Posicao"))) ) { 
				  ...
				  veicED.setOid_Veiculo(Integer.valueOf(pneuED.getOid_Veiculo()));
	...
	public ArrayList listaUltima_Recapagem(PneuED ed) throws Excecoes {
		...
		try {
			...
			" Pneus.oid_Empresa = " + ed.getOid_Empresa() + " " ;
			 if ("true".equals(ed.getDm_Virada())){
				  sql += " and Pneus.vl_Ultima_Recapagem > 0 " ;
			  }
  ...
  private PneuED doGetByRecord (PneuED ed) throws Excecoes {
    ArrayList<PneuED> lista = this.lista_Consulta (ed);
    Iterator<PneuED> iterator = lista.iterator ();
    if (iterator.hasNext ()) {
  ...
  public PneuED getByRecordOL (PneuED ed) throws Excecoes {
	ArrayList<PneuED> lista = this.lista (ed);
	Iterator<PneuED> iterator = lista.iterator ();
	if (iterator.hasNext ()) {
		return (PneuED) iterator.next ();
	}
  ...
  public ArrayList<PneuED> listaDescrita (PneuED ed) throws Excecoes {
    ArrayList<PneuED> list = new ArrayList<PneuED> ();
			  sql = "SELECT " +
					"p.*, " +
				    "f.cd_fabricante_pneu, " +
  ...
  public void geraRelPneus (PneuED ed , HttpServletResponse res) throws Excecoes {
    ArrayList<PneuED> lista = this.listaDescrita (ed);
  ...
  */
    private PneuED populaRegistro(ResultSet rs) throws SQLException, Excecoes {
		...
		if (ed.getNr_Vida()>0) {
		  ...
		  //recED = new RecapagemBD(this.executaSQL).getLastRecord(recED);
		  //ed.setNm_Fabricante_Ultima_Recapagem(recED.getNm_Fabricante_Banda());
		  //ed.setNm_Modelo_Pneu_Ultima_Recapagem(recED.getNm_Banda());
		}
		
		ed.setDt_Estoque(FormataData.formataDataBT(rs.getString("dt_Estoque")));
		if (doValida(ed.getDt_Estoque())) { // Verifica se a data de entrada é valida 
			...
			//ed.setOid_Unidade_Sucateamento(leED.getOid_Unidade()); // Usado para quando for sucatear  apartir do estoque
		}
		// Pegando informaçoes da frota
		ed.setDt_Entrada(FormataData.formataDataBT(rs.getString("dt_Entrada")));
		if (doValida(ed.getDt_Entrada())) { // Verifica se a data de entrada é valida
		...
		if (doValida(ed.getDt_Remessa_Recapagem())) { // Verifica se a data de entrada é valida 
		...
		if (doValida(ed.getDt_Sucateamento())) { // Verifica se a data de entrada é valida
			...
			//mot_sucED.setOid_Empresa(ed.getOid_Empresa());
			...
			//	}
			//}
			//if (doValida(fornED.getOid_Fornecedor())){
			//	fornED = new FornecedorBD(this.executaSQL).getByRecord(fornED);
		...
		if (doValida(ed.getDt_Venda())) { // Verifica se a data de entrada é valida 
		...
		ed.setMsg_Stamp(("I".equals(rs.getString("dm_Stmp"))? "Incluído":"Alterado") + " por " + rs.getString("usu_Stmp")+ " em " + FormataData.formataDataBT(rs.getString("dt_Stmp")));
		return ed;
  }
    /**
    ...
    public void atualizaSituacao (PneuED ed) throws Excecoes {
    	try {
    		...
    		edBusca.setOid_Pneu((int)oidPneu); // ed exclusivo para buscar o pneu ( só tem o oid_pneu preenchido )
    		
    		// Pega o pneu antes da atualizacao para o registro da medicao
    		//PneuED pneuAntesED = this.getByRecordOL(edBusca);
    		...
    		if ( doValida(ed.getDt_Sucateamento()) )
    			...
    			",oid_Motivo_Sucateamento="+ed.getOid_Motivo_Sucateamento()+
    		 	",oid_Unidade_Sucateamento="+ed.getOid_Unidade_Sucateamento();
    		else
    			sql+=",dt_Sucateamento=null" +
    			",oid_Motivo_Sucateamento=null"+
    			",oid_Unidade_Sucateamento=null";
    ...
    public void registraRecusaRecapagem (PneuED ed) throws Excecoes {
  	  try {
  		  ...
  		  throw new Excecoes (e.getMessage () , e , this.getClass ().getName () , "registraRecusaRecapagem (PneuED ed)");
  	  }
    }
     ...
     * @throws Excecoes
     */
    public ArrayList<PneuED> analiseSucata (PneuED ed,String pWhere) throws Excecoes {
    	ArrayList<PneuED> list = new ArrayList<PneuED> ();
    	sql = "SELECT " +
    	"p.oid_Pneu, " +
    	"p.nr_Fogo, " +
    	...
    	try {
    		...
    		while (rs.next ()) {
    			...
    			//ArrayList<Vida_PneuED> lst = new Vida_PneuBD(this.executaSQL).lista(vidaED);
...
//    			}
    ...
    public ArrayList<PneuED> analiseSucataResumo1 (PneuED ed,String pWhere) throws Excecoes {
    	ArrayList<PneuED> list = new ArrayList<PneuED> ();
    	...
    	try {
    		...
    		while (rs1.next ()) {
        		...
        		while (rs2.next ()) {
        			...
        			ArrayList<PneuED> lstCtaVida = new ArrayList<PneuED>();
        			while  (rs4.next ()) {
        				// Calcula percentual de pneus sucateados em relação aos que estão rodando na frota com a mesma dimensao
    ...
    public ArrayList<PneuED> analiseSucataResumo2 (PneuED ed,String pWhere) throws Excecoes {
    	ArrayList<PneuED> list = new ArrayList<PneuED> ();
    ...
    public ArrayList<PneuED> analiseSucataResumo3 (PneuED ed,String pWhere) throws Excecoes {
    	ArrayList<PneuED> list = new ArrayList<PneuED> ();
    	try {
    ...
    public ArrayList<PneuED> analiseSucataResumo4 (PneuED ed,String pWhere) throws Excecoes {
    	ArrayList<PneuED> list = new ArrayList<PneuED> ();
    	try {
    ...
    public ArrayList<PneuED> analiseSucataResumo5 (PneuED ed,String pWhere) throws Excecoes {
    	ArrayList<PneuED> list = new ArrayList<PneuED> ();
    	try {
	    	//Retorna quantos pneus foram sucateados em cada marca
	    	sql = "SELECT " +
    ...
    public void aJustaMMs (PneuED ed) throws Excecoes {
    	try {
	    	sql = "SELECT * " +
	    	"FROM " +
	    	"pneus p " +
	    	...
	    	"p.oid_empresa = " +ed.getOid_Empresa() + 
	    	" order by oid_Pneu";
			ResultSet rsPn = this.executaSQL.executarConsulta(sql);
			//long i=0;
			while (rsPn.next ()) {
				//System.out.println("STGPOL - #ajustaMM " + i++ + " " + rsPn.getString("oid_pneu"));
				sql = "select * " +
					  "from " +
					  "Medicoes_Pneus as mp " +
					  "where " +
					  "oid_pneu ='"+rsPn.getString("oid_pneu")+"' " +
			  		  "order by " +
			  		  "mp.dt_Medicao_Pneu desc";
				ResultSet rsM = this.executaSQL.executarConsulta(sql);
				if (rsM.next ()) {
					sql = " UPDATE "+
       		        "pneus "+
    		        " SET "+
			        " mm_atual ="+ rsM.getDouble("nr_Mm_Sulco") +
			        ",nr_mm_Inicial ="+ rsM.getDouble("nr_Mm_Sulco") +
			        " WHERE "+   	
			        "oid_pneu = '"+ rsPn.getString("oid_pneu")+"' ";
					executaSQL.executarUpdate (sql);
				}
