...
public class Menu_PerfilBD extends BancoUtil {
	...
	public Menu_PerfilED inclui(Menu_PerfilED ed) throws Excecoes {
		...
		try {
			...
			// Inclui para este perfil todas as opções de menu do sistema
			// Precisa ler as opções do sistema (menus_sistemas) e iterar sobre ele incluido em opçoes do perfil (menus_perfis_sistemas)  
			...
			ArrayList lst = new Menu_SistemaBD(this.executasql).lista(msED); 	// Busca a lista de opçoes do menu do sistema
			for (int i=0 ; i<lst.size() ; i++ ){								// Itera sobre a lista de opções de menu do sistema
				msED = (Menu_SistemaED)lst.get(i);								// Instancia um ed para opçes do perfil
				Menu_Perfil_SistemaED mpsED = new Menu_Perfil_SistemaED();      // Monta o ed com os dados das opções de menu do sistema 
	...
	public void altera(Menu_PerfilED ed) throws Excecoes {
		...
		try {
			sql = "UPDATE Menus_Perfis SET " +
				  "oid_Sistema = " + ed.getOid_Sistema() + " " +
				  ",nm_Menu_Perfil = '" + ed.getNm_Menu_Perfil() + "' " +
			...
			for (int x=0;x<lst.size();x++) {
				Menu_Perfil_SistemaED mpED = (Menu_Perfil_SistemaED)lst.get(x);
				new Menu_Perfil_SistemaBD(this.executasql).altera(mpED);		// Inclui a opção de menu do sistema no perfil.
			}

		} catch (Exception exc) {
			throw new Excecoes(exc.getMessage(), exc, this.getClass().getName(),"altera(Menu_PerfilED ed)");
		}
	}
	...
	public void deleta(Menu_PerfilED ed) throws Excecoes {
		...
		try {
			...
			// Deletar todas as opções de menu do perfil ( menus_perfis_sistemas )
