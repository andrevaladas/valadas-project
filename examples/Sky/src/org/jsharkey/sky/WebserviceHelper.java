...
public class WebserviceHelper {
	...
	public static final class ForecastParseException extends Exception {
		public ForecastParseException(final String detailMessage) {
			super(detailMessage);
		}

		public ForecastParseException(final String detailMessage, final Throwable throwable) {
	 ...
	 */
	private static long parseDate(final String raw) throws TimeFormatException {
		// Inject milliseconds so that NWS dates follow RFC
		sEditable.clear();
		sEditable.append(raw);
		sEditable.insert(19, ".000");

		final String rfcFormat = sEditable.toString();
	 ...
	 * returned.
	 */
	private static Forecast getForecast(final Map<String, List<Forecast>> forecasts,
			final String layout, final int index) {
		if (!forecasts.containsKey(layout)) {
			forecasts.put(layout, new ArrayList<Forecast>());
		}
		final List<Forecast> layoutSpecific = forecasts.get(layout);
	 ...
	 */
	private static List<Forecast> flattenForecasts(final Map<String, List<Forecast>> forecasts) {
		final List<Forecast> flat = new ArrayList<Forecast>();

		// Collect together all forecasts that have valid conditions
		for (final String layout : forecasts.keySet()) {
			for (final Forecast forecast : forecasts.get(layout)) {
		...
		Collections.sort(flat, new Comparator<Forecast>() {
			@Override
			public int compare(final Forecast left, final Forecast right) {
				if (left.alert) {
					return -1;
				} else {
	 ...
	 */
	public static void updateForecasts(final Context context, final Uri appWidgetUri, final int days)
			throws ForecastParseException {

		final Uri appWidgetForecasts = Uri.withAppendedPath(appWidgetUri, AppWidgets.TWIG_FORECASTS);
		...
		final List<Forecast> forecasts = queryLocation(lat, lon, days);
		...
		// before today
		final long lastMidnight = ForecastUtils.getLastMidnight();
	 ...
	 */
	private static List<Forecast> queryLocation(final double lat, final double lon, final int days)
			throws ForecastParseException {
		...
		if (FAKE_DATA) {
			...
			final HttpGet request = new HttpGet(String.format(WEBSERVICE_URL, lat, lon, days));
	 ...
	 */
	private static List<Forecast> parseResponse(final Reader response)
			throws ForecastParseException {
		// Keep a temporary mapping between time series tags and forecasts
		final Map<String, List<Forecast>> forecasts = new HashMap<String, List<Forecast>>();
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (TAG_TEMPERATURE.equals(thisTag) || TAG_WEATHER.equals(thisTag)
						...
						final Forecast forecast = getForecast(forecasts, thisLayout, ++index);
